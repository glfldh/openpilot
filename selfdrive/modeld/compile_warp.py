...
def make_update_img_input(frame_prepare, model_w, model_h):
  def update_img_input_tinygrad(tensor, frame, M_inv):
    ...
    # TODO can we have n images a dim on the buffers, can we store one less image?
    full_buffer = tensor[6:].cat(new_img, dim=0).contiguous() # TODO can we assign instead of returning?
...
def run_and_save_pickle():
  for cam_w, cam_h in CAMERA_CONFIGS:
    # compile_modeld_warp(cam_w, cam_h) # TODO folded in policy run, remove make update_both_images, make sure SConscript flags are correct
